...
def bedtoolsFunction(command):
	...
	file.write('\tcmd = paste0(getOption(\"bedtools.path\", default=\".\"), "/bedtools ' + infoDict["ToolName"].rstrip() + ' ", options' + cmdstring + ', " > ", tempfile) \n\tsystem(cmd) \n')
...
.onLoad <- function(libname, pkgname) {
  ...
  if(length(row)>0) {
    ...
    response<-tryCatch({
      system(paste0(getOption("bedtools.path", default="."), "/bedtools --version"), intern=T)
    }, error = function(e) {
